...
app.post('/', function (req, res) {
    ...
    db.collection('dogs', function (err, collection) {
        collection.insert(req.body, function (err, result) { //inserting the info to database - adding the dog to the database
            console.log('Inserted %d documents into the "dogs" collection:', result.length, result);
        });
        collection.find().toArray(function (err, items) { //you have to find all the dogs
            res.render('index', { //send them back to the home page with all the dogs
                users: items
